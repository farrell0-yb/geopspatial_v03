...
--    1) lm_st_convexhull       -> ST_ConvexHull
--    2) lm_st_intersection     -> ST_Intersection  (convex polygons)
--    3) lm_st_union            -> ST_Union          (bounding approach)
--    4) lm_st_difference       -> ST_Difference     (convex polygons)
--    5) lm_st_symdifference    -> ST_SymDifference
--    6) lm_st_buffer           -> ST_Buffer         (approximate)
--    7) lm_st_isvalid          -> ST_IsValid
--    8) lm_st_touches          -> ST_Touches
--    9) lm_st_crosses          -> ST_Crosses
--   10) lm_st_overlaps         -> ST_Overlaps
--   11) lm_st_equals           -> ST_Equals
--   12) lm_st_simplify_vw     -> ST_SimplifyVW (Visvalingam-Whyatt)
--
-- ============================================================================


-- ============================================================
-- 1)  lm_st_convexhull  —  PostGIS ST_ConvexHull equivalent
--     Computes the convex hull using the Graham scan algorithm.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_convexhull(p_geom geometry)
RETURNS geometry
...
-- Example:
--   SELECT lm_st_astext(lm_st_convexhull(
--       lm_make_polygon(ARRAY[-1,0,1,0], ARRAY[0,1,0,-1])));


-- ============================================================
-- 2)  lm_st_intersection  —  PostGIS ST_Intersection equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_intersection(p_a geometry, p_b geometry)
RETURNS geometry
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
-- ============================================================
-- 3)  lm_st_union  —  PostGIS ST_Union equivalent
...
--     (an outer bound).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_union(p_a geometry, p_b geometry)
RETURNS geometry
LANGUAGE sql IMMUTABLE
AS $$
   SELECT lm_st_convexhull(
      ROW((p_a).lon || (p_b).lon,
...
-- ============================================================
-- 4)  lm_st_difference  —  PostGIS ST_Difference equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_difference(p_a geometry, p_b geometry)
RETURNS geometry
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF NOT lm_st_intersects(p_a, p_b) THEN RETURN p_a; END IF;
   IF lm_st_contains(p_b, p_a) THEN
      RETURN ROW(ARRAY[]::double precision[], ARRAY[]::double precision[])::geometry;
   END IF;
...
-- ============================================================
-- 5)  lm_st_symdifference  —  PostGIS ST_SymDifference
--     Parts in A or B but not both.
--     Returns the convex hull of the two differences combined.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_symdifference(p_a geometry, p_b geometry)
RETURNS geometry
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
   diff_ab geometry := lm_st_difference(p_a, p_b);
   diff_ba geometry := lm_st_difference(p_b, p_a);
...
-- ============================================================
-- 6)  lm_st_buffer  —  PostGIS ST_Buffer equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_buffer(
   p_geom geometry,
...
-- ============================================================
-- 7)  lm_st_isvalid  —  PostGIS ST_IsValid equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_isvalid(p_geom geometry)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF lm_st_area(p_geom) = 0 THEN RETURN false; END IF;
...
-- ============================================================
-- 8)  lm_st_touches  —  PostGIS ST_Touches equivalent
--     Returns true if geometries have at least one boundary
--     point in common, but their interiors do not intersect.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_touches(p_a geometry, p_b geometry)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF NOT lm_st_intersects(p_a, p_b) THEN RETURN false; END IF;
...
-- ============================================================
-- 9)  lm_st_crosses  —  PostGIS ST_Crosses equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_crosses(p_a geometry, p_b geometry)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF na = 2 AND nb = 2 THEN
      RETURN lm_st_intersects(p_a, p_b)
         AND NOT lm_st_contains(p_a, p_b)
         AND NOT lm_st_contains(p_b, p_a);
   END IF;
...
-- ============================================================
-- 10) lm_st_overlaps  —  PostGIS ST_Overlaps equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_overlaps(p_a geometry, p_b geometry)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF dim_a >= 2 THEN
      IF lm_st_contains(p_a, p_b) OR lm_st_contains(p_b, p_a) THEN
         RETURN false;
      END IF;
   END IF;
...
-- ============================================================
-- 11) lm_st_equals  —  PostGIS ST_Equals equivalent
...
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_equals(p_a geometry, p_b geometry)
RETURNS boolean
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF na >= 3 AND nb >= 3 THEN
      RETURN lm_st_contains(p_a, p_b) AND lm_st_contains(p_b, p_a);
   END IF;
...
-- ============================================================
-- 12) lm_st_simplify_vw  —  PostGIS ST_SimplifyVW equivalent
--     Visvalingam-Whyatt simplification.
--     Removes vertices based on effective triangle area.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_simplify_vw(
