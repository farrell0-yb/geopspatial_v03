...
--
-- PostGIS-equivalent "LM" functions.  Every function has two overloads:
--   - Array style:    lm_st_contains(lon_a[], lat_a[], lon_b[], lat_b[])
--   - Geometry style: lm_st_contains(a geometry, b geometry)
--
...
--   1) lm_st_xmin        -> min longitude
--   2) lm_st_xmax        -> max longitude
--   3) lm_st_ymin        -> min latitude
--   4) lm_st_ymax        -> max latitude
--   5) lm_st_translate    -> shifted polygon / geometry
--   6) lm_st_intersects   -> boolean
--   7) lm_st_contains     -> boolean
--
...
--   -- Returns: -112.0
--
CREATE OR REPLACE FUNCTION lm_st_xmin(p_lon double precision[])
...
BEGIN
   IF n = 0 THEN RAISE EXCEPTION 'lm_st_xmin: lon array must not be empty'; END IF;
   result := p_lon[1];
   FOR i IN 2..n LOOP
...
--
-- Example:
--   SELECT lm_st_xmin(lm_make_polygon(
--       ARRAY[-112.0, -111.9, -111.9, -112.0],
--       ARRAY[40.5,   40.5,   40.55,  40.55]));
--   -- Returns: -112.0
--
CREATE OR REPLACE FUNCTION lm_st_xmin(p_geom geometry)
RETURNS double precision LANGUAGE sql IMMUTABLE
AS $$ SELECT lm_st_xmin((p_geom).lon); $$;
...
-- Array version
CREATE OR REPLACE FUNCTION lm_st_xmax(p_lon double precision[])
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
   ...
   v double precision; result double precision; i integer;
BEGIN
   IF n = 0 THEN RAISE EXCEPTION 'lm_st_xmax: lon array must not be empty'; END IF;
   result := p_lon[1];
   FOR i IN 2..n LOOP
      v := p_lon[i]; IF v > result THEN result := v; END IF;
   END LOOP;
...
-- Geometry version
CREATE OR REPLACE FUNCTION lm_st_xmax(p_geom geometry)
RETURNS double precision LANGUAGE sql IMMUTABLE
AS $$ SELECT lm_st_xmax((p_geom).lon); $$;


-- ============================================================
-- 3)  lm_st_ymin  --  PostGIS ST_YMin equivalent
-- ============================================================

-- Array version
CREATE OR REPLACE FUNCTION lm_st_ymin(p_lat double precision[])
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
   ...
   v double precision; result double precision; i integer;
BEGIN
   IF n = 0 THEN RAISE EXCEPTION 'lm_st_ymin: lat array must not be empty'; END IF;
   result := p_lat[1];
   FOR i IN 2..n LOOP
...
-- Geometry version
CREATE OR REPLACE FUNCTION lm_st_ymin(p_geom geometry)
RETURNS double precision LANGUAGE sql IMMUTABLE
AS $$ SELECT lm_st_ymin((p_geom).lat); $$;


-- ============================================================
-- 4)  lm_st_ymax  --  PostGIS ST_YMax equivalent
-- ============================================================

-- Array version
CREATE OR REPLACE FUNCTION lm_st_ymax(p_lat double precision[])
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
   ...
   v double precision; result double precision; i integer;
BEGIN
   IF n = 0 THEN RAISE EXCEPTION 'lm_st_ymax: lat array must not be empty'; END IF;
   result := p_lat[1];
   FOR i IN 2..n LOOP
      v := p_lat[i]; IF v > result THEN result := v; END IF;
   END LOOP;
...
-- Geometry version
CREATE OR REPLACE FUNCTION lm_st_ymax(p_geom geometry)
RETURNS double precision LANGUAGE sql IMMUTABLE
AS $$ SELECT lm_st_ymax((p_geom).lat); $$;
...
--       lm_make_polygon(ARRAY[-112.0, -111.9, -111.9, -112.0],
--                       ARRAY[40.5,   40.5,   40.55,  40.55]),
--       0.01, -0.02);
--
CREATE OR REPLACE FUNCTION lm_st_translate(
...
BEGIN
   IF n = 0 OR n <> coalesce(array_length(src_lat, 1), 0) THEN
      RAISE EXCEPTION 'lm_st_translate: geometry must not be empty';
   END IF;
   FOR i IN 1..n LOOP
...
--       lm_make_polygon(ARRAY[-112.0, -111.9, -111.9, -112.0],
--                       ARRAY[40.5,   40.5,   40.55,  40.55]),
--       lm_make_polygon(ARRAY[-111.95, -111.85, -111.85, -111.95],
--                       ARRAY[40.52,   40.52,   40.57,   40.57]));
--   -- Returns: true
--
CREATE OR REPLACE FUNCTION lm_st_intersects(p_a geometry, p_b geometry)
RETURNS boolean LANGUAGE sql IMMUTABLE
AS $$
   SELECT lm_st_intersects((p_a).lon, (p_a).lat, (p_b).lon, (p_b).lat);
$$;
...
-- Example:
--   SELECT lm_st_contains(
--       lm_make_polygon(ARRAY[-112.0, -111.8, -111.8, -112.0],
--                       ARRAY[40.4,   40.4,   40.6,   40.6]),
--       lm_make_polygon(ARRAY[-111.95, -111.85, -111.85, -111.95],
--                       ARRAY[40.45,   40.45,   40.55,   40.55]));
--   -- Returns: true
--
CREATE OR REPLACE FUNCTION lm_st_contains(p_a geometry, p_b geometry)
RETURNS boolean LANGUAGE sql IMMUTABLE
AS $$
   SELECT lm_st_contains((p_a).lon, (p_a).lat, (p_b).lon, (p_b).lat);
$$;
