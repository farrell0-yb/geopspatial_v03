...
--    1) lm_st_distance          -> ST_Distance
--    2) lm_st_length            -> ST_Length
--    3) lm_st_perimeter         -> ST_Perimeter
--    4) lm_st_centroid          -> ST_Centroid
--    5) lm_st_distancesphere    -> ST_DistanceSphere
--    6) lm_st_dwithin           -> ST_DWithin
--    7) lm_st_simplify          -> ST_Simplify
--    8) lm_st_lineinterpolatepoint  -> ST_LineInterpolatePoint
--    9) lm_st_linelocatepoint   -> ST_LineLocatePoint
--   10) lm_st_linesubstring     -> ST_LineSubstring
--   11) lm_st_geomfromtext      -> ST_GeomFromText
--   12) lm_st_geomfromgeojson   -> ST_GeomFromGeoJSON
--   13) lm_st_rotate            -> ST_Rotate
--   14) lm_st_affine            -> ST_Affine
--   15) lm_st_dumppoints        -> ST_DumpPoints
--   16) lm_st_dumpsegments      -> ST_DumpSegments
--   17) lm_st_snaptogrid        -> ST_SnapToGrid
--   18) lm_st_removerepeatedpoints -> ST_RemoveRepeatedPoints
--   19) lm_st_segmentize        -> ST_Segmentize
--   20) lm_st_clipbybox2d       -> ST_ClipByBox2D
--   21) lm_st_generatepoints    -> ST_GeneratePoints
--   22) lm_st_chaikinsmoothing  -> ST_ChaikinSmoothing
--   23) lm_st_expand            -> ST_Expand
--   24) lm_st_summary           -> ST_Summary
--   25) lm_st_addpoint          -> ST_AddPoint
--   26) lm_st_removepoint       -> ST_RemovePoint
--   27) lm_st_setpoint          -> ST_SetPoint
--   28) lm_st_project           -> ST_Project
--
-- ============================================================================
...
-- ============================================================
-- 1)  lm_st_distance  —  PostGIS ST_Distance equivalent
--     Minimum planar distance between two geometries.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_distance(p_a geometry, p_b geometry)
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF lm_st_intersects(p_a, p_b) THEN RETURN 0; END IF;
...
-- ============================================================
-- 2)  lm_st_length  —  PostGIS ST_Length equivalent
--     Returns the planar length of a linestring / polygon
--     perimeter.  Sum of all segment lengths.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_length(p_geom geometry)
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
-- ============================================================
-- 3)  lm_st_perimeter  —  PostGIS ST_Perimeter equivalent
--     Sum of all edge lengths of a polygon (closed ring).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_perimeter(p_geom geometry)
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   RETURN lm_make_point(cx / area6, cy / area6);
END;
$$;


-- ============================================================
-- 5)  lm_st_distancesphere  —  PostGIS ST_DistanceSphere
--     Great-circle distance in meters using the Haversine formula.
--     Assumes coordinates are lon/lat in degrees (SRID 4326).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_distancesphere(p_a geometry, p_b geometry)
RETURNS double precision
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
-- Example:
--   SELECT lm_st_distancesphere(
--       lm_make_point(-104.9903, 39.7392),   -- Denver
--       lm_make_point(-111.8910, 40.7608));   -- Salt Lake City
--   -- Returns: ~596,000 meters


-- ============================================================
-- 6)  lm_st_dwithin  —  PostGIS ST_DWithin equivalent
--     Returns true if the planar distance between two
--     geometries is less than or equal to the threshold.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_dwithin(
   p_a geometry, p_b geometry, p_distance double precision
)
RETURNS boolean
LANGUAGE sql IMMUTABLE
AS $$
   SELECT lm_st_distance(p_a, p_b) <= p_distance;
$$;
...
-- ============================================================
-- 7)  lm_st_simplify  —  PostGIS ST_Simplify equivalent
--     Douglas-Peucker line simplification.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_simplify(
...
-- ============================================================
-- 8)  lm_st_lineinterpolatepoint  —  PostGIS equivalent
--     Returns a point at a given fraction (0..1) along a line.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_lineinterpolatepoint(
...
BEGIN
   ...
   FOR i IN 1..n-1 LOOP
      ...
      IF running + seg_len >= target_len THEN
         IF seg_len = 0 THEN
            RETURN lm_make_point((p_geom).lon[i], (p_geom).lat[i]);
         END IF;
   ...
   -- Fraction = 1.0
   RETURN lm_make_point((p_geom).lon[n], (p_geom).lat[n]);
END;
$$;


-- ============================================================
-- 9)  lm_st_linelocatepoint  —  PostGIS equivalent
--     Returns fraction (0..1) of the closest point on line
--     to the given point.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_linelocatepoint(
...
-- ============================================================
-- 10) lm_st_linesubstring  —  PostGIS equivalent
--     Returns the portion of a line between two fractions.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_linesubstring(
   p_geom geometry,
...
-- ============================================================
-- 11) lm_st_geomfromtext  —  PostGIS ST_GeomFromText equivalent
--     Parses WKT strings: POINT, LINESTRING, POLYGON.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_geomfromtext(p_wkt text)
RETURNS geometry
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   IF wkt LIKE 'POINT(%' THEN
      ...
      RETURN lm_make_point(parts[1]::double precision, parts[2]::double precision);
   ...
   ELSE
      RAISE EXCEPTION 'lm_st_geomfromtext: unsupported WKT type: %', p_wkt;
   END IF;
END;
$$;

-- Example:
--   SELECT lm_st_geomfromtext('POINT(-111.97 40.52)');
--   SELECT lm_st_geomfromtext('POLYGON((-112 40,-111 40,-111 41,-112 41,-112 40))');


-- ============================================================
-- 12) lm_st_geomfromgeojson  —  PostGIS ST_GeomFromGeoJSON
--     Parses a GeoJSON geometry object (text or jsonb).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_geomfromgeojson(p_json text)
RETURNS geometry
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
BEGIN
   ...
   IF gtype = 'Point' THEN
      ...
      RETURN lm_make_point((coords->>0)::double precision,
   ...
   ELSE
      RAISE EXCEPTION 'lm_st_geomfromgeojson: unsupported type: %', gtype;
   END IF;
END;
$$;

-- Example:
--   SELECT lm_st_geomfromgeojson('{"type":"Point","coordinates":[-111.97,40.52]}');


-- ============================================================
-- 13) lm_st_rotate  —  PostGIS ST_Rotate equivalent
--     Rotates geometry by angle (radians) around a center point.
--     Default center: origin (0,0).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_rotate(
   p_geom  geometry,
...
-- ============================================================
-- 14) lm_st_affine  —  PostGIS ST_Affine equivalent (2D)
--     Applies 2D affine transformation:
--       x' = a*x + b*y + xoff
--       y' = d*x + e*y + yoff
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_affine(
   p_geom geometry,
...
-- ============================================================
-- 15) lm_st_dumppoints  —  PostGIS ST_DumpPoints equivalent
--     Returns a set of (path, geom) for each vertex.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_dumppoints(p_geom geometry)
...
BEGIN
   FOR i IN 1..n LOOP
      path := ARRAY[i];
      geom := lm_make_point((p_geom).lon[i], (p_geom).lat[i]);
      RETURN NEXT;
   END LOOP;
END;
$$;
...
-- ============================================================
-- 16) lm_st_dumpsegments  —  PostGIS ST_DumpSegments equivalent
--     Returns a set of (path, geom) for each edge segment.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_dumpsegments(p_geom geometry)
RETURNS TABLE(path integer[], geom geometry)
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
...
-- ============================================================
-- 17) lm_st_snaptogrid  —  PostGIS ST_SnapToGrid equivalent
--     Rounds all coordinates to the nearest multiple of size.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_snaptogrid(
...
-- ============================================================
-- 18) lm_st_removerepeatedpoints  —  PostGIS equivalent
--     Removes consecutive duplicate vertices.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_removerepeatedpoints(
...
-- ============================================================
-- 19) lm_st_segmentize  —  PostGIS ST_Segmentize equivalent
--     Adds intermediate vertices so that no segment exceeds
--     max_segment_length (planar).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_segmentize(
...
-- ============================================================
-- 20) lm_st_clipbybox2d  —  PostGIS ST_ClipByBox2D equivalent
--     Clips a polygon to an axis-aligned bounding box using
--     the Sutherland-Hodgman algorithm.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_clipbybox2d(
   p_geom geometry,
...
LANGUAGE sql IMMUTABLE
AS $$
   SELECT lm_st_clipbybox2d(
      p_geom,
      lm_st_xmin(p_box), lm_st_ymin(p_box),
      lm_st_xmax(p_box), lm_st_ymax(p_box));
$$;


-- ============================================================
-- 21) lm_st_generatepoints  —  PostGIS ST_GeneratePoints
--     Generates N random points inside a polygon using
--     rejection sampling.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_generatepoints(
...
AS $$
DECLARE
   xmin double precision := lm_st_xmin(p_geom);
   xmax double precision := lm_st_xmax(p_geom);
   ymin double precision := lm_st_ymin(p_geom);
   ymax double precision := lm_st_ymax(p_geom);
   count integer := 0;
   px double precision; py double precision;
BEGIN
   WHILE count < p_npoints LOOP
      ...
      IF point_in_polygon(px, py, (p_geom).lon, (p_geom).lat) THEN
         RETURN NEXT lm_make_point(px, py);
         count := count + 1;
      END IF;
   END LOOP;
END;
...
-- ============================================================
-- 22) lm_st_chaikinsmoothing  —  PostGIS ST_ChaikinSmoothing
--     Applies Chaikin's corner-cutting algorithm.
--     nIterations defaults to 1.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_chaikinsmoothing(
...
AS $$
   SELECT lm_make_bbox(
      lm_st_xmin(p_geom) - p_amount,
      lm_st_ymin(p_geom) - p_amount,
      lm_st_xmax(p_geom) + p_amount,
      lm_st_ymax(p_geom) + p_amount
   );
$$;


-- ============================================================
-- 24) lm_st_summary  —  PostGIS ST_Summary equivalent
--     Returns a text summary describing the geometry.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_summary(p_geom geometry)
RETURNS text
LANGUAGE plpgsql IMMUTABLE
AS $$
DECLARE
   ...
   gtype text;
BEGIN
   gtype := lm_geometry_type(p_geom);
   RETURN gtype || '[' || n || ' vertices'
      || ', bbox(' || lm_st_xmin(p_geom)::text || ' '
      || lm_st_ymin(p_geom)::text || ', '
      || lm_st_xmax(p_geom)::text || ' '
      || lm_st_ymax(p_geom)::text || ')]';
END;
$$;

-- Example:
--   SELECT lm_st_summary(lm_make_bbox(-112, 40, -111, 41));
--   -- Returns: 'POLYGON[4 vertices, bbox(-112 40, -111 41)]'


-- ============================================================
-- 25) lm_st_addpoint  —  PostGIS ST_AddPoint equivalent
--     Adds a point to a geometry at a given position (0-based).
--     If position is omitted, appends to the end.
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_addpoint(
...
-- ============================================================
-- 26) lm_st_removepoint  —  PostGIS ST_RemovePoint equivalent
--     Removes the vertex at the given index (0-based).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_removepoint(
...
BEGIN
   IF idx < 1 OR idx > n THEN
      RAISE EXCEPTION 'lm_st_removepoint: index % out of range [0..%]', p_index, n-1;
   END IF;
...
-- ============================================================
-- 27) lm_st_setpoint  —  PostGIS ST_SetPoint equivalent
--     Replaces the vertex at the given index (0-based).
-- ============================================================
CREATE OR REPLACE FUNCTION lm_st_setpoint(
...
BEGIN
   IF idx < 1 OR idx > n THEN
      RAISE EXCEPTION 'lm_st_setpoint: index % out of range [0..%]', p_index, n-1;
   END IF;
...
BEGIN
   ...
   RETURN lm_make_point(degrees(lon2), degrees(lat2));
END;
$$;
...
--   SELECT lm_st_astext(
--      lm_st_project(lm_make_point(-104.9903, 39.7392), 1000, 0));
